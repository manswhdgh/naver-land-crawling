...
class RealEstateApp:
    ...
    def create_widgets(self):
        ...
        Label(self.root, text="최소 가격 (억)").grid(row=3, column=0)
        self.minPrice = StringVar()
        Entry(self.root, textvariable=self.minPrice).grid(row=3, column=1)

        Label(self.root, text="최대 가격 (억)").grid(row=4, column=0)
    ...
    def start_scraping(self):
        ...
        minPrice = int(self.minPrice.get()) * 10000
        maxPrice = int(self.maxPrice.get()) * 10000
...
def get_real_address(latitude, longitude):
    geolocator = Nominatim(user_agent="nvcraw")
